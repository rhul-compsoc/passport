...
const microsoftStrategy = new OIDCStrategy(
  {
    ...
    redirectUrl: configuration.location.origin + "/login/" + UserConnectionPlatform.MICROSOFT + "/return",
    responseType: "code",
    scope: ["email", "profile"],
  ...
  async (req: Request, profile: MicrosoftProfile, done: VerifyCallback) => {
    ...
    if (!profile._json.email)
      return done(
        new Error(
          "First.Last.1992@live.rhul.ac.uk style email missing from `_json` field."
        )
      );
    ...
    if (!profile._json.preferred_username)
      return done(
        new Error(
          "XXXX000@live.rhul.ac.uk style email missing from `_json` field."
        )
      );

    if (!rhulEmailRegex.test(profile._json.preferred_username))
      return done(
        new Error("Malformed email, or not a RHUL University email address.")
      );
